...
import org.openhab.binding.lightwaverf.internal.handler.FeatureSetHandler;
import org.openhab.binding.lightwaverf.internal.handler.LWAccountHandler;
import org.openhab.binding.lightwaverf.internal.handler.StructureHandler;
...
public class LWHandlerFactory extends BaseThingHandlerFactory {
    ...
    protected @Nullable ThingHandler createHandler(Thing thing) {
        ...
        if (THING_TYPE_LIGHTWAVE_ACCOUNT.equals(thingTypeUID)) {
            ...
            return handler;
        } else if (THING_TYPE_LIGHTWAVE_STRUCTURE.equals(thingTypeUID)) {
            StructureHandler handler = new StructureHandler(thing);
            return handler;
        } else if (THING_TYPE_LIGHTWAVE_DEVICE.equals(thingTypeUID)) {
            DeviceHandler handler = new DeviceHandler(thing);
            return handler;
        } else if (THING_TYPE_LIGHTWAVE_SOCKET.equals(thingTypeUID)) {
            FeatureSetHandler handler = new FeatureSetHandler(thing);
            return handler;
        } else if (THING_TYPE_LIGHTWAVE_DIMMER.equals(thingTypeUID)) {
            FeatureSetHandler handler = new FeatureSetHandler(thing);
            return handler;
        } else if (THING_TYPE_LIGHTWAVE_THERMOSTAT.equals(thingTypeUID)) {
            DeviceHandler handler = new DeviceHandler(thing);
            return handler;
        } else if (THING_TYPE_LIGHTWAVE_HUB.equals(thingTypeUID)) {
            DeviceHandler handler = new DeviceHandler(thing);
            return handler;
                  }            
        else {return null;}
