...
/**
 * The {@link ReceiverThread} class handles communication received
...
public class ReceiverThread implements Runnable {
    ...
    private final CopyOnWriteArrayList<StringMessageListener> listeners = new CopyOnWriteArrayList<StringMessageListener>();
    private boolean running = false;
    protected byte[] data = new byte[1024];
    protected DatagramSocket socket;

    public ReceiverThread(StringMessageListener listener) {
        listeners.add(listener);
    }

    public synchronized void stopReceiver(StringMessageListener listener) {
    ...
    public void run() {
        ...
        try {
            ...
            logger.debug("Receive Thread Socket exception:{}", e);
            running = false;
        } catch (IOException e) {
            logger.debug("Receive Thread IO exception:{}", e);
            running = false;
        }
    }
